...
from .policies import ALIASES, REQUIRED_FIELDS, authorize_target_write, classify_header, normalize_header
...
class MigrationEngine:
    ...
    def seed_mapping(self, run_id: str) -> dict[str, Any] | None:
        ...
        if not run:
            return None
        run["mapping"], run["escalations"] = self._map_headers(run.get("source_headers", []))
    ...
    def _map_headers(self, headers: list[str]) -> tuple[dict[str, str], list[dict[str, Any]]]:
        mapping, escalations = {}, []
        for header in sorted(set(headers)):
            if header == "_source":
                continue
            if classify_header(header) == "sensitive":
                escalations.append({"id": str(uuid.uuid4()), "type": "sensitive_column", "field": header, "status": "open", "reason": "Unexpected sensitive source column was quarantined"})
                continue
            ...
            if len(candidates) == 1:
                ...
                escalations.append({"id": str(uuid.uuid4()), "type": "ambiguous_mapping", "field": header, "candidates": candidates, "status": "open", "reason": "Top mapping candidates have insufficient confidence margin"})
    ...
    def _normalise(record: dict[str, str]) -> dict[str, str]:
        ...
        if output.get("start_date"):
            for fmt in ("%Y-%m-%d", "%d/%m/%Y", "%m/%d/%Y"):
                try:
                    output["start_date"] = datetime.strptime(output["start_date"], fmt).date().isoformat()
                    break
                except ValueError:
                    pass
        return output
